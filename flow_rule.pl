...
do_del(N, T, V, Pin, Pout, S, Snew) :-
    (do(del(N, T, V, Pin, Pout), S, SN), Snew = SN);
        Snew = S.
...
:- do(pr(pi2, po1), s0, S4), do_del(*, *, "p", *, *,S4, SN), output(SN).
:- do(pr(pi2, po1), s0, S4), do_del(*, *, "pppp", *, *,S4, SN), output(SN).
